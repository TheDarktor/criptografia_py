...
if funcao == 1:
    texto = str(input('Insira um texto (somente letras minúsculas): ')) # Usuário insere um texto para ser criptografado
    t = ''
    ...
    if x == 1:
        ...
        texto = str(input('Insira um texto (somente letras minúsculas): ')) # Usuário insere um texto para ser descriptografado
        t = ''
        for l in texto: # as letras do texto são separadas e analisadas individualmente
            if l in alfabeto:
                l_index = alfabeto.index(l) # l_index é o index da letra que está sendo analisada
